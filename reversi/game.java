...
public class game {
	/*
	 * we need to return a NEW board for a legal move, and return NULL if NO legal move. 
	 * 		we can have a boolean that checks for legal moves - if boolean is TRUE, we return board, else return NULL.
	 */
	...
	public static board checkRight(board board, int[] coordinates, int player) {
		//return NULL if no legal move found, otherwise change the board to our new board and return that
		int row = coordinates[0];
		int col = coordinates[1];
		
		if (col >= board.boardSize) return null; //if column is too far beyond and we havent found an answer
		else if (board.board[row][col] == 0) return null; //if we encounter an empty slot
		else if (board.board[row][col] == player) return board;
...
//			System.out.println("Changed " + row + "," + col + " to " + player+ " with checkRight");
	...
	public static board checkLeft(board board, int[] coordinates, int player) {
		...
		else if (board.board[row][col] == 0) return null;
		else if (board.board[row][col] == player) return board;
		else if (board.board[row][col] == returnOpponent(player)) {
...
//			System.out.println("Changed " + row + "," + col + " to " + player+ " with checkUp");
	...
	public static board checkUpRight(board board, int[] coordinates, int player) {
		...
		if (row < 0 || col >= board.boardSize) return null;
		else if (board.board[row][col] == 0) return null;
		else if (board.board[row][col] == player) return board;
		else if (board.board[row][col] == returnOpponent(player)) {
//			System.out.println("Changed " + row + "," + col + " to " + player+ " with checkUpRight");
	...
	public static board checkDownLeft(board board, int[] coordinates, int player) {
		...
		if (row >=board.boardSize || col < 0) return null;
		else if (board.board[row][col] == 0) return null;
		else if (board.board[row][col] == player) return board;
		else if (board.board[row][col] == returnOpponent(player)) {
...
//			System.out.println("Changed " + row + "," + col + " to " + player+ " with checkUpLeft");
	...
	public static board checkDownRight(board board, int[] coordinates, int player) {
		...
		if (row >=board.boardSize || col >=board.boardSize) return null;
		else if (board.board[row][col] == 0) return null;
		else if (board.board[row][col] == player) return board;
		else if (board.board[row][col] == returnOpponent(player)) {
//			System.out.println("Changed " + row + "," + col + " to " + player+ " with checkDownRight");
	...
	public static board checkDown(board board, int[] coordinates, int player) {
		...
		if (row>=board.boardSize) return null;
		else if (board.board[row][col] == 0) return null; //if we encounter an empty slot
		else if (board.board[row][col] == player) return board;
		else if (board.board[row][col] == returnOpponent(player)) {
//			System.out.println("Changed " + row + "," + col + " to " + player+ " with checkDown");
	...
	public static board makeMove(board board, int[] coordinates, int player) {
		...
		//if the given coordinate is NOT empty, its an illegal move so we return a null board
		if (auxBoard.board[row][col]!=0) 
			return null;
		else auxBoard.board[row][col] = player;
		
		//check right - remember that the check right method ALTERS the actual board in case of a legal move, 
		//so to avoid setting auxBoard to null, we can use a temporary one to do it for us.
...
//					System.out.println("Has Legal Move DownRight");
	...
	public static int[] inputStringToArray(String input) {//convert input string to matrix coordinates
		...
		//aux[0] will carry the column, aux[1] will carry the row, we must reverse it for the coordinate
...
//		System.out.println("Input Matrix Coordinates: " + coordinate[0] + "," + coordinate[1]);
	...
	public static boolean isGameOver(board board) {
		if(!canWhiteMove(board) && !canBlackMove(board)) return true;
	...
	public static void main(String [] args) {
		...
		while(!isGameOver(board)) {
			if (player == board.BLACK) {
					...
					else System.out.println("Black (x) plays."); 
				}
				...
				else System.out.println("White (o) plays");
			}
			...
			board tmp = makeMove(board.copy(), inputStringToArray(input), player);
		...
		if (board.score_white == board.score_black) System.out.println("Draw");
		else if (board.score_white > board.score_black) System.out.println("White Wins");
